...
def create_achievements():
    ...
    if achievement is None:
        ...
        achievement = Achievement(
            name="Elon's Helper", description="Find and talk to XB1."
        )
        db.session.add(achievement)
        db.session.commit()
    ...
    if "postType" in data:
        if "login" == data["postType"]:
            ...
            if exists:
                ...
                return {
                    "error": "A user with that username and password does not exist."
                }
            ...
            try:
                ...
                if "UNIQUE constraint failed" in str(e) or "Duplicate entry" in str(e):
                    ...
                    return {"error": str(e)}
            response = {"user": new_user.username, "user-id": new_user.id}
            return response
            ...
            achievement_obj = Achievement.query.filter_by(
                id=int(data["achievement_id"])
            ).first()
